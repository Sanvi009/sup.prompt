...
export default async function handler(req, res) {
  ...
  // Verify authentication (optional for guest feed)
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    try {
      ...
      // Get user's activity to determine category preferences
      if (userId) {
        ...
        if (activity && activity.length > 0) {
          // Count category occurrences from likes and saves
          ...
          // Sort categories by frequency (most liked/saved first)
  ...
  if (req.method === 'GET' && action === 'feed') {
    ...
    // Combine boosted + regular
    let allPrompts = [...boosted, ...regular];

    // 3. Apply personalization for logged-in users
    if (userId && userCategories.length > 0) {
      // Get user's liked/saved prompt IDs to filter them out
      ...
      // - Category match (weighted by user preference)
      // - From following (highest priority)
      // - Not already liked/saved (bonus)
      const scored = allPrompts.map(prompt => {
        let score = 0;

        // Boost from following (highest priority)
        if (followingIds.length > 0) {
          // Check if prompt creator is in following list
          // Note: We don't have creator_id in prompts table yet
          // For now, we'll skip this or add later
        }
        ...
        if (prompt.category_ids && prompt.category_ids.length > 0) {
          ...
          // Weight: higher for more matched categories
        ...
        // Boosted prompts get extra score
    ...
    // 4. Get like/save/comment counts for each prompt
    const promptsWithCounts = await Promise.all(
      allPrompts.map(async (prompt) => {
        ...
        // Check if user liked/saved this prompt (if logged in)
        let liked = false;
        let saved = false;
        if (userId) {
  ...
  if (req.method === 'GET' && action === 'search') {
    ...
    // 1. Search users (first 5 results)
    ...
    // Get like/save counts for prompts
    const promptsWithCounts = await Promise.all(
  ...
  // ============================================
  // EXPLORE (Guest feed - no personalization)
  // ============================================
